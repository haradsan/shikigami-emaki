...
let _rollWait = null;      // 現在アクティブな「賽を振る」待ち（投了時に安全に中断するため）
let _surrendering = false; // 投了確認ダイアログの二重表示防止

// ---------- ゲーム開始 ----------
async function startGame(stageIdx, opts = {}) {
  document.body.classList.add("in-game"); // 上部の情報窓・下段（手札／賽）を表示
  ...
  G.training = !!opts.training; // 稽古（練習対戦・進行度を更新せず勝利でカード3枚）
  G.hotseat = !!opts.versus;    // 2人対戦（ホットシート・報酬/進行度なし）
  G.royale = !!opts.royale;     // 三つ巴（人間1+CPU2・勝利でカード獲得・進行度は変化しない）
  G.sealed = !!opts.sealed;     // 封符戦（その場開封のプールで構築・勝利でカード獲得・進行度は変化しない）
  G.sealedDeck = opts.sealedDeck || null; // 「同じデッキでもう一度」用に封符戦デッキを保持
  G.versusSetup = opts.versus || null; // 「もう一度」用に対戦設定を保持
  GAME_SPEED = G.training ? 0.28 : 1; // 稽古は演出を高速化（時短）。通常は等倍
  // 稽古は目標資産・ラウンド上限を下げて短時間で決着（時短・簡易）
  if (G.training) {
  ...
  // ※ v26まではここで中央HUD（メッセージ／賽）の位置を stage.hud でずらしていた。
  //    v27で賽と操作ボタンを下段の操作ドックへ移し、盤面の上には何も置かなくなったので不要
  ...
  log(`総資産 ${RULES.target}G に達して本宮に戻れば勝利です（霊力が尽きても敗北にはならず、本宮で再起できます）`, "sys");
  // 対戦前の口上: 相手キャラのポートレートと挨拶（稽古・2人対戦・リトライでは省略）
...
async function showGameOver() {
  ...
  // 封符戦: 勝てばカードを獲得（進行度は変化しない）。開封プールは使い捨てなので
  // 「同じデッキでもう一度」でデッキだけ引き継いで再戦できる
  if (G.sealed) {
    setMessage(youWin ? "🎁 封符戦を制覇！" : `💀 ${G.winner.name}の勝利…`);
    if (youWin) await playVictoryFx("VICTORY!", "🎁 封符戦を制覇！");
    let gained = 0;
    if (youWin && typeof grantWinCards === "function") {
      const set = await chooseRewardSet();
      const pack = grantWinCards(REWARD_WIN, set);
      if (pack) { gained = pack.length; await showPackReveal(pack, "🏆 封符戦 勝利報酬！", `カードを${pack.length}枚手に入れた！（こちらはコレクションに入ります）`); }
    }
    const res = await showDialog({
      title: youWin ? "🏆 封符戦を制覇！" : "💀 敗北…",
      body: `${esc(G.winner.name)}の勝ち！ あなた ${assetsOf(G, G.players[0])}G ／ ${esc(G.players[1].name)} ${assetsOf(G, G.players[1])}G` +
        ...
        `<br><br>（封符戦の開封プールは使い捨て・進行度は変化しません）`,
      buttons: [
  ...
  if (G.royale) {
    ...
    // 週替わりの神事適用中の三つ巴勝利にもボーナスカード（正規対戦と同様・v22）
  ...
  // 稽古（練習対戦）: 進行度は更新せず、勝てばカードを3枚獲得（時短仕様）
  if (G.training) {
    ...
    if (youWin && typeof grantTrainingCards === "function") {
      ...
      await showPackReveal(cards,
        streak >= 2 ? `🎯 稽古 🔥${streak}連勝！` : "🎯 稽古勝利！",
        `報酬としてカードを${gained}枚獲得！` +
        (streak >= TRAINING_STREAK_FOR_RARE ? `（🔥${streak}連勝ボーナス＝稀以上1枚保証）`
          : streak === TRAINING_STREAK_FOR_RARE - 1 ? `（次も勝てば🔥${TRAINING_STREAK_FOR_RARE}連勝＝稀以上保証！）` : ""));
    ...
    const res = await showDialog({
      ...
      body: `${esc(G.winner.name)}の勝ち！ あなた ${assetsOf(G, G.players[0])}G ／ ${esc(G.players[1].name)} ${assetsOf(G, G.players[1])}G` +
        (youWin ? `<br><br>💚 カードを${gained}枚獲得しました（📚アルバムで確認できます）` +
          (streak >= 1 ? `<br>🔥 稽古${streak}連勝中！ ${TRAINING_STREAK_FOR_RARE}連勝からは毎回<b>稀以上1枚保証</b>` : "")
        : `<br><br>🔥 連勝は途切れた…（稽古の連勝ボーナスはリセット）`),
      buttons: [
  ...
  // 報酬: 新規クリア=10枚 / クリア済みステージの再勝利=5枚。文箱の巻は勝者が選ぶ（v19）
  ...
  if (firstClear && typeof grantStageClearPack === "function") {
    ...
    if (pack) await showPackReveal(pack, "🎁 STAGE 初クリア報酬！", `大型文箱（${pack.length}枚）を手に入れた！`);
  ...
  // 週替わりの神事適用中の正規勝利にはボーナスカードを追加（クリア済みステージ再訪の動機に）
  if (youWin && G.weekly && typeof drawPack === "function") {
  ...
  const res = await showDialog({
    ...
    body: `${esc(G.winner.name)}の勝ち！<br>あなたの総資産: ${assetsOf(G, G.players[0])}G ／ ${esc(G.players[1].name)}: ${assetsOf(G, G.players[1])}G` +
      ...
      (youWin && !hasNext ? `<br><br>👑 <b>全ステージ制覇！ あなたは真の陰陽師だ！</b>` : "") +
      quoteHtml,
    buttons,
  });
...
async function chooseRewardSet() {
  ...
  const row = s => {
    ...
    return { label: `${s.icon} ${s.name}文箱（収集 ${have}/${inSet.length}種）`, value: String(s.set) };
  };
  const res = await showDialog({
    title: "🎁 報酬文箱の巻を選択",
    body: "どちらの巻の文箱を受け取りますか？",
...
// ---------- 途中棄権（投了） ----------
// 安全に投了できるのは「自分のターンで賽を振る前」だけ（_rollWait が有効なとき）。
// その瞬間だけ投了ボタンを表示し、進行中の賽待ちを中断してタイトルへ戻す。
...
async function requestSurrender() {
  ...
  if (!_rollWait) return; // 自分の番で賽を振る前だけ受け付ける
  _surrendering = true;
  // 投了するのは「いま手番で賽待ちの人間」（2人対戦ではどちらのプレイヤーもあり得る）
  const loser = G.players[G.current];
  const winner = opponentOf(G, loser);
  const res = await showDialog({
    title: `🏳 ${G.hotseat ? `${loser.name}は` : ""}投了しますか？`,
    body: G.training
      ? "この稽古を中断します（報酬なし・進行度は変わりません）。"
      : G.hotseat
  ...
  // 進行中の賽待ちを無効化（旧ターンのコルーチンが新ゲームへ波及しないように）
...
async function playTurn(p) {
  ...
  // 3. 移動（通過ボーナス処理込み）。通過マスは式神侵攻の出撃元判定に使う
  ...
  if (p.timeRitual && !G.over) {
    ...
    log(`🕯️ 時の儀！ ${p.name}はもう一度動く——賽: ${dice2}`, "warn");
    p.lastPath = [];
...
function turnStartTick(p) {
  ...
  if (veinGain > 0) { p.magic += veinGain; SFX.coin(); log(`💎 霊力鉱脈から${p.name}に+${veinGain}G`); }
  // 🌸春の芽吹き（v20）: 2Rの間、自軍式神をターン開始時に+15回復
...
// 「あとは本宮へ帰るだけ」の状態を全員に見えるようにして、凱旋レースの緊張感を作る
function notifyReach() {
  ...
  G.players.forEach(p => {
    ...
    if (reached && !p.reached) {
      p.reached = true;
      SFX.coin();
      log(`⚑ ${p.name}は目標資産${RULES.target}Gに到達！ 🏯本宮へ凱旋すれば勝利だ！`, "warn");
...
// ---------- 人間: 呪術使用（1ターン1回・任意）→ 賽ボタン ----------
async function humanSpellAndRoll(p) {
  let spellUsed = false;
  // 🗺マップ確認モード中は手札が畳まれている＝呪術を選べないので、自分の操作に入る前に解除する
  // （見たいときにまた🗺を押せばよい。見えない手札を探させないための保険）
  exitMapFocus();
  while (true) {
    renderHand(G);
    if (!spellUsed) markCastableSpells(p);
    const rollWait = waitButtonCancellable("🎲 賽を振る");
...
// キャンセル可能なボタン待ち（呪術使用で賽待ちを中断するため）。
// ボタンの実体は showActionButton が決める（🎲ラベルなら操作ドックの丸い賽ボタン＝ui.js）
...
async function humanPickOpponent(p, title, body, filterFn = null) {
  ...
  const res = await showDialog({
    ...
    buttons: cands.map(q => ({
      label: `${P_ICONS[q.id]} ${q.name}（霊力${q.magic}G／総資産${assetsOf(G, q)}G／手札${q.hand.length}枚）`,
      value: String(q.id),
...
// 三つ巴では「相手」を選ぶ呪術（ドレイン等）は対象プレイヤーを選択する（2人対戦では従来どおり自動）
// 呪術は「何が起きたか」がログにしか出ない手が多いので、castSpell の実行中だけ
// 「ログ＝ポップアップにも出す」スコープを張る（ui.js beginLogToast）。
// こうしておけば新しい呪術を足したときに toast の付け忘れが起きない。
...
async function castSpellEffect(p, cardId) {
  ...
  const opp = opponentOf(G, p); // 筆頭の相手（総資産トップ）。意趣返しの符・劣勢判定の基準
  if (c.cost > p.magic) return false;
  // 🤫沈黙の霧（v20）: このターンは呪術を使えない
  if (p.spellSealed) {
    if (!p.isCPU) log(`🤫 沈黙の霧に閉ざされている——このターンは呪術を使えない`, "warn");
    return false;
  }
  // 🌙静寂のとばり（v20）: 対象指定の呪術は全員使用不可
  if (TARGETED_SPELLS.has(c.spell) && activeFx(G, "silence")) {
    if (!p.isCPU) log(`🌙 静寂のとばり——対象を指定する呪術は今は使えない`, "warn");
    return false;
  }
  ...
  if (c.spell === "quake") {
    ...
    const target = p.isCPU ? richestOpponent(G, p)
      : await humanPickOpponent(p, "💰 収奪の符 — 相手を選択", "所持金の半分を奪う相手を選んでください");
    if (!target) return false;
    pay();
    ...
    p.magic += amount;
    SFX.coin();
    log(`💰 ${p.name}の収奪の符！ ${target.name}の所持金の半分 ${amount}G を奪った`, "warn");
    ...
    if (arr !== "lap" && !p.isCPU) log("（鳥居が揃っていないため周回ボーナス（霊力・回復）は無し。霊地の采配は発動）", "warn");

  } else if (c.spell === "revenge") {
    const gap = assetsOf(G, opp) - assetsOf(G, p);
    if (gap <= 0) { if (!p.isCPU) log("意趣返しの符は総資産で負けている時のみ使えます", "warn"); return false; }
    pay();
    const amount = Math.min(Math.floor(gap * 0.25), 500, opp.magic);
    opp.magic -= amount;
    p.magic += amount;
    log(`✨ ${p.name}の意趣返しの符！ 資産差${gap}G — ${opp.name}から${amount}Gを奪った`, "warn");
    ...
    if (p.isCPU) {
      ...
      dst = await humanPickLand(gustDests(G, src),
        ...
        "移動先の空き地を選んでください（相手の式神がそのマスへ移り、元の土地は空き地に戻ります）",
        "移動先の空き地がありません");
      if (!dst) return false;
    }
    pay();
    ...
    // 自分の式神1体を盤面の好きな空き地へ転送（現在HPごと・元の土地は空き地に戻る）。不動は対象外
    ...
    if (p.isCPU) {
      ...
      src = await humanPickLand(movable,
        "🚪 遷座の符 — 転送する式神を選択",
        "盤面の<b>好きな空き地</b>へ転送する自分の式神を選んでください（<b>不動は対象外</b>）<br>元の土地は空き地に戻ります（レベルは残る）",
        "転送できる式神がいません（不動は対象外）");
      if (!src) return false;
      dst = await humanPickLand(empties,
        `🚪 ${CARD_BY_ID[src.creature.cardId].name} をどこへ転送する？`,
        "転送先の空き地を選んでください（式神が現在HPのまま移り、その土地を入手します）",
        "転送先の空き地がありません");
      if (!dst) return false;
    }
    pay();
    ...
    src.owner = null;
    src.creature = null;
    log(`🚪 ${p.name}の遷座の符！ ${moved.name}が${tileName(dst)}へ転送された（元の土地は空き地に）`);

  } else if (c.spell === "leap") {
    // 自分の式神1体を「ちょうど2マス先」の空き地へ跳躍させる。不動は対象外
    const movable = ownedLands(G, p.id).filter(t =>
    ...
    if (p.isCPU) {
      ...
      dst = await humanPickLand(leapDests(G, src),
        ...
        "2マス先の空き地から跳躍先を選んでください（式神が現在HPのまま移り、その土地を入手します）",
        "跳躍先の空き地がありません");
      if (!dst) return false;
    }
    pay();
    ...
    p.hand.push(target);
    log(`♻️ ${p.name}の反故拾い！ 捨て札から${CARD_BY_ID[target].name}を手札に戻した`);
    await enforceHandLimit(p);
  ...
  // --- 移動 ---
    ...
    const target = await humanPickTileOnMap(dests, {
      title: "↩️ 後退り — 戻るマスを選択",
      body: "自分のコマを<b>1〜3マス後ろ</b>へ戻します（移動のみ＝マスの効果・鳥居は発動しません）。そのあと通常どおり賽で移動します。",
      cancelable: true,
      labelFn: t => t.type === "LAND"
        ? `${ELEMENTS[t.element].icon} ${tileName(t)}${t.owner !== null ? `（${t.owner === p.id ? "自領" : "敵領"} Lv${t.level}）` : "（空き地）"}`
        : `#${t.id} ${tileName(t)}`,
    ...
    log(`↩️ ${p.name}の後退り！ ${tileName(target)}へ戻った`);
    renderBoard(G);
    ...
    if (mine.length < 2) { if (!p.isCPU) log("入れ替える式神が2体いません", "warn"); return false; }
    const a = await humanPickLand(mine, "🔀 集結 — 1体目を選択", "位置を入れ替える自分の式神を2体選びます（1体目）", "");
    if (!a) return false;
    ...
    const target = await humanPickOpponent(p, "♟️ 入れ替えの呪 — 相手を選択", "コマの位置を入れ替える相手を選んでください");
    if (!target) return false;
    pay();
    const tmp = p.pos;
    p.pos = target.pos;
    ...
    log(`♟️ ${p.name}の入れ替えの呪！ ${target.name}とコマの位置が入れ替わった`, "warn");
    renderBoard(G);

  } else if (c.spell === "reverse") {
    // 🔄 時流逆転（v24）: プレイヤー1人（自分も可）の進行方向を反転させる。
    // 通常移動は順方向のみ（v24）のため、逆走はこの呪術と🎋おみくじマスの渦でしか起きない
    let target;
    if (p.isCPU) {
    ...
    // 🧪 増殖の秘薬（v24）: 自分の場の式神1体と同名のカードを手札に加える
    // （🐺百鬼・🫧分裂・🔁転生などと組むコンボ用。コレクションには影響しない＝対戦内だけの複製）
    const mine = ownedLands(G, p.id).filter(t => t.creature);
    let target;
    if (p.isCPU) {
      ...
      target = await humanPickLand(mine, "🧪 増殖の秘薬 — 対象を選択",
        "自分の場の式神1体を選ぶと、<b>同名カード1枚</b>が手札に加わります（🐺百鬼・🫧分裂と好相性）",
        "自分の式神がいません");
      if (!target) return false;
    }
    pay();
    ...
    const target = p.isCPU ? aiPickDeportTarget(G, p)
      : await humanPickOpponent(p, "🏯 強制送還 — 相手を選択", "本宮へ送り返す相手を選んでください（周回はつきません）", q => q.pos !== 0);
    if (!target) { if (!p.isCPU) log("送還できる相手がいません（すでに本宮）", "warn"); return false; }
    pay();
    target.pos = 0;
    target.lastPath = [];
    target.cameFrom = null; // 本宮からの再出発＝方向リセット
    log(`🏯 ${p.name}の強制送還！ ${target.name}は本宮へ送り返された（周回はつかない）`, "warn");
    renderBoard(G);
  ...
  // --- バトル支援 ---
    ...
    pay();
    p.nextBattleSt = 20;
    log(`📣 ${p.name}の鬨の声！ 次のバトルでST+20`);

  } else if (c.spell === "guardwind") {
    pay();
    p.nextDefHp = 20;
    log(`🌬️ ${p.name}の護りの風！ 次に防衛する自軍式神のHP+20`);
    ...
    const target = p.isCPU ? aiPickBlessingTarget(G, p)
      : await humanPickLand(cands, "🕊️ 言祝ぎ — 対象を選択",
        "自軍式神1体を<b>永続強化</b>: ST/最大HP+10（🌱成長と同じ枠＝合計+25まで）",
        "強化できる式神がいません（すでに最大強化）");
    if (!target) return false;
    pay();
    ...
    log(`🕊️ ${p.name}の言祝ぎ！ ${bc.name}が祝福された（ST/最大HP+${Math.min(5, target.creature.grown) * 5}の強化状態）`);
  ...
  // --- 土地 ---
    ...
    const target = await humanPickLand(ownedLands(G, p.id),
      "💱 高値売却 — 対象を選択", `自分の土地1つを<b>価値の${Math.round(HIGHSELL_RATE * 100)}%</b>で売却します（通常の強制売却は${Math.round(SELL_RATE * 100)}%。駐留式神は手札に戻ります）`,
      "売却できる土地がありません");
    if (!target) return false;
    pay();
    ...
    const target = p.isCPU ? aiPickVeinTarget(G, p)
      : await humanPickLand(ownedLands(G, p.id).filter(t => t.veinOwner === undefined || t.veinOwner === null),
        "💎 鉱脈発見 — 対象を選択", "自分の土地1つに<b>霊力鉱脈</b>を付与: 以後、自分のターン開始時<b>+20G</b>（永続・土地を失うと消える）",
        "鉱脈を掘れる土地がありません");
    if (!target) return false;
    pay();
    target.veinOwner = p.id;
    log(`💎 ${p.name}は${tileName(target)}に霊力鉱脈を発見！ 以後ターン開始時+20G`);
    ...
    const target = p.isCPU ? aiPickCurselandTarget(G, p)
      : await humanPickLand(enemyLandsOf(G, p).filter(t => !landSpellShielded(G, t) && !landCursed(G, t)),
        "🕯️ 祟り地の呪 — 対象を選択", `敵の土地1つの<b>通行料を半減</b>します（${OVERLAY_DURATION}ラウンド）`,
        "呪える敵の土地がありません");
    if (!target) return false;
    pay();
    target.curseUntil = G.round + OVERLAY_DURATION - 1;
    log(`🕯️ ${p.name}の祟り地の呪！ ${tileName(target)}の通行料が半減（${OVERLAY_DURATION}ラウンド）`, "warn");
  ...
  // --- 妨害 ---
    ...
    const target = await humanPickOpponent(p, "🕵️ 間者の術 — 相手を選択", "手札をすべて見る相手を選んでください", q => q.hand.length > 0);
    if (!target) { if (!p.isCPU) log("手札を持っている相手がいません", "warn"); return false; }
    pay();
    log(`🕵️ ${p.name}の間者の術！ ${target.name}の手札を覗き見た`);
    await showDialog({
    ...
    const target = p.isCPU ? aiPickSlowTarget(G, p)
      : await humanPickOpponent(p, "🎲 呪い賽 — 相手を選択", "次の出目を1〜3にする相手を選んでください");
    if (!target) return false;
    pay();
    target.diceCurse = true;
    log(`🎲 ${p.name}の呪い賽！ ${target.name}の次の出目は1〜3になる`, "warn");
    ...
    const target = p.isCPU ? richestOpponent(G, p)
      : await humanPickOpponent(p, "🔥 霊力焼きの呪 — 相手を選択", "霊力の20%を消滅させる相手を選んでください（上限300G）");
    if (!target) return false;
    const amt = Math.min(300, Math.floor(target.magic * 0.2));
    if (amt <= 0) { if (!p.isCPU) log("相手に燃やせる霊力がありません", "warn"); return false; }
    pay();
    target.magic -= amt;
    log(`🔥 ${p.name}の霊力焼きの呪！ ${target.name}の霊力${amt}Gが燃え尽きた（誰の物にもならない）`, "warn");
    ...
    const target = p.isCPU ? aiPickNullfogTarget(G, p)
      : await humanPickLand(
        ...
        "🌫️ 無力化の霧 — 対象を選択", `敵式神1体の<b>能力をすべて消します</b>（${OVERLAY_DURATION}ラウンド。宝具で得る能力は消えない）※<b>護法</b>持ちは対象外`,
        "対象にできる敵式神がいません");
    if (!target) return false;
    pay();
    ...
    const target = p.isCPU ? aiPickSilenceTarget(G, p)
      : await humanPickOpponent(p, "🤫 沈黙の霧 — 相手を選択", "次のターン、呪術を使えなくする相手を選んでください");
    if (!target) return false;
    pay();
    target.spellSealed = true;
    log(`🤫 ${p.name}の沈黙の霧！ ${target.name}は次のターン呪術を使えない`, "warn");
    ...
    log(`🧊 ${p.name}の大金縛り！ ${targets.map(q => q.name).join("・")}は次のターン動けない`, "warn");

  } else if (c.spell === "miragefield") {
    pay();
    addFx(G, "mirage", p.id, OVERLAY_DURATION);
    log(`🏜️ ${p.name}の蜃気楼！ ${OVERLAY_DURATION}ラウンドの間、自分の土地は敵の土地対象呪術の対象にならない`);
  ...
  // v25: 潤沢の儀＝神饌の儀の派生。1〜3枚を好きなだけ捧げ、1枚につき+300G（1枚あたりは神饌の儀より割安）
    ...
    if (creatures.length === 0 || empties.length === 0) {
      if (!p.isCPU) log("蘇生できる式神（捨て札）か空き地がありません", "warn");
      return false;
    }
    let reviveId, dst;
    if (p.isCPU) {
    ...
    if (victims.length === 0) { if (!p.isCPU) log("対象にできる敵式神がいません", "warn"); return false; }
    const sac = await ritualSacrifice(p, cardId, c.name);
    if (!sac) return false;
    pay();
    discardFromHand(p, sac);
  ...
  // --- 盤面エフェクト（全体・2R） ---
    ...
    log(`🏪 ${p.name}の市場開放！ ${OVERLAY_DURATION}Rの間、札マスで2枚ドロー（全員）`);
    ...
    log(`⚡ ${p.name}の霊力嵐！ ${OVERLAY_DURATION}Rの間、すべての通行料1.5倍`, "warn");

  } else if (c.spell === "fx_silence") {
    pay();
    addFx(G, "silence", p.id, OVERLAY_DURATION);
    log(`🌙 ${p.name}の静寂のとばり！ ${OVERLAY_DURATION}Rの間、対象指定の呪術は全員使えない`, "warn");

  } else if (c.spell === "fx_goddess") {
    pay();
    addFx(G, "goddess", p.id, OVERLAY_DURATION);
    log(`👼 ${p.name}の神域の加護！ ${OVERLAY_DURATION}Rの間、自分の土地の加護2倍＋援護ST+10`);
  }
...
// 呪術対象の土地を選ぶ（人間用）。盤面から直接クリックでも選べる。候補なし/キャンセルなら null
...
async function movePlayer(p, steps) {
  ...
  for (let i = 0; i < steps; i++) {
    ...
    if (tile.type === "GATE" && !p.gates.has(tile.id)) {
      ...
      SFX.coin();
      log(`⛩️ ${p.name}は鳥居をくぐった +${RULES.gateBonus}G`);
      renderPanels(G);
    }
    if (tile.type === "CASTLE") {
      // 最後の1歩でぴったり本宮に停止したか（それ以外は「通過」）
...
function arriveCastle(g, p, exact = true) {
  if (assetsOf(g, p) >= RULES.target) {
    renderAll(g);
    endGame(p, `${p.name}は総資産${assetsOf(g, p)}Gで本宮に帰還！`);
    return "win";
  }
  let result = null;
  if (p.gates.size >= gatesNeededOf(g)) {
    ...
    healAllCreatures(g, p); // 周回ボーナス＝霊力＋全回復（本宮の通過・停止どちらでも）
    log(`🏯 ${p.name}は周回達成！ +${bonus.gold}G — 自軍式神のHPが全回復！`, "", { toast: true });
    if (typeof cpuSay === "function") cpuSay(p, "lap");
    result = "lap";
  }
  if (exact) {
    // 本宮にぴったり停止＝霊地の采配。周回していなくても、拠点で軍に指示を出せる
    p.passAllLands = true;
    log(`🏯 ${p.name}は本宮に留まり軍を指揮——全ての自領で1回行動できる！（霊地の采配）`, "", { toast: true });
...
// 指定プレイヤーの全式神のHPを最大に戻す（周回ボーナス）。成長分（v19）も含めた実最大HPまで
...
// true のときだけ②通過アクションは行わない。カード/霊力/神隠し/火口/鳥居/本宮などの受け身マスは
// false（＝このターン通過した自領があれば②通過アクションを選べる）。
async function tileAction(p, tile) {
  switch (tile.type) {
    case "CARD": {
      // 🏪市場開放（v20）: 2Rの間、札マスで2枚ドロー（全員）
      ...
      log(`🎴 ${p.name}は札マスで${got ? `${got}枚ドロー${n === 2 ? "（🏪市場開放）" : ""}` : "…山札切れ"}`);
      await enforceHandLimit(p);
      return false;
    }
    case "MAGIC":
      ...
      SFX.coin();
      log(`💎 ${p.name}は霊力マスで+${RULES.magicTileG}G`);
      return false;
    case "WARP": {
      await sleep(300);
      p.pos = tile.warpTo;
      p.cameFrom = null; // 神隠し先では方向未確定＝どの方向へも出発できる
      p.lastPath.push(p.pos);
      SFX.spell();
      log(`🌀 ${p.name}は神隠しした！`);
      renderBoard(G);
    ...
    case "MAGMA": {
      ...
      p.magic -= loss;
      SFX.hit();
      log(`🌋 ${p.name}は火口で足止め… -${loss}G`, "warn");
      return false;
    }
    case "BOOST": {
      SFX.dice();
    ...
    case "FORTUNE": {
      // 🎋 おみくじマス: 何が出るかはお楽しみ（期待値はややプラス・15%で凶）
      SFX.dice();
      log(`🎋 ${p.name}はおみくじを引いた——`);
      await sleep(550);
      const r = Math.random();
      if (r < 0.10) {
        p.magic += 300; SFX.coin();
        log(`🎉 大吉！ 神々の祝福で +300G！`, "warn");
      } else if (r < 0.35) {
        p.magic += 150; SFX.coin();
        log(`💰 吉！ +150G`, "", { toast: true });
      } else if (r < 0.60) {
        const a = drawCard(G, p), b = drawCard(G, p);
        log(`🎴 おみくじの導き！ カードを${(a ? 1 : 0) + (b ? 1 : 0)}枚ドロー`, "", { toast: true });
        if (a && !p.isCPU) await animateDraw(CARD_BY_ID[a]);
        if (b && !p.isCPU) await animateDraw(CARD_BY_ID[b]);
        await enforceHandLimit(p);
      } else if (r < 0.80) {
        p.diceMult = 2;
        log(`🎲 追い風の予感！ 次の賽の出目が2倍になる`, "", { toast: true });
      } else if (r < 0.90) {
        // 🌀 時空の渦（v24）: 進行方向が反転するイベント（逆走は呪術とこれでのみ起きる）
    ...
    case "SPRING": {
      // ♨️ 霊泉マス: 自軍式神全回復＋少額の霊力（周回を待たずに前線を立て直せる）
      const wounded = G.tiles.filter(t =>
        t.type === "LAND" && t.owner === p.id && t.creature && isWounded(t.creature)).length;
      healAllCreatures(G, p);
      p.magic += 60;
      SFX.coin();
      log(`♨️ ${p.name}は癒しの霊泉で安らいだ +60G${wounded ? `・負傷式神${wounded}体が全回復！` : ""}`, "", { toast: true });
      return false;
    }
    case "CASTLE":
    case "GATE":
...
// ② 通過アクション: 通過地レベルアップ / 通過式神の侵攻 / 通過地の式神交代（どれか1つ、任意）
async function passActionPhase(p) {
  ...
  while (true) {
    ...
    const lines = [];
    if (canMarch) {
      buttons.push({ label: "🏇 式神侵攻", value: "march" });
      lines.push("・<b>式神侵攻</b>：通過した自分の式神を隣のマスへ進める（行軍費を支払う）");
    }
    if (canSwap) {
      buttons.push({ label: "🔄 式神交代", value: "swap" });
      lines.push("・<b>式神交代</b>：通過した自分の土地の駐留式神を手札の式神と入れ替える");
    }
    if (canPassUp) {
    ...
    const res = await showDialog({
      title: all ? "🏯 霊地の采配（全自領が対象）" : "🏇 通過アクション（このターンの行動権をここで使う／任意）",
      body: (all
        ? "<b>本宮に停止（霊地の采配）！</b> <b>すべての自分の霊地</b>について次のいずれか1つを行えます（使わないなら「何もしない」）。<br>"
        : "①到達マスでは能動的な行動をしなかったので、<b>このターンの能動行動の権利</b>が残っています。<br>" +
          "このターンに<b>通過した</b>自分の霊地について、次のいずれか1つに使えます（使わないなら「何もしない」）。<br>" +
...
async function humanPassLevelupFlow(p) {
  ...
  const t = await humanPickTileOnMap(sources, {
    ...
    body: p.passAllLands
      ? "強化する自分の土地を1つ選んでください（霊地の采配＝全ての自領が対象）"
...
// 通過した自分の土地の駐留式神を手札の式神と交代（②通過アクション1件）
function doPassSwap(p, tile, cardId) {
  const c = CARD_BY_ID[cardId];
  if (c.cost > p.magic) {
    log(`⚠ 霊力が足りず交代できない（${c.name} ${c.cost}G／霊力 ${p.magic}G）`, "warn");
    return;
  }
...
// 人間用: 通過した自分の土地→手札式神を選んで交代。キャンセルなら false
async function humanPassSwapFlow(p) {
  ...
  if (sources.length === 1) {
    ...
    src = await humanPickTileOnMap(sources, {
      title: "🔄 式神交代 — 土地を選択",
      body: p.passAllLands
        ? "駐留式神を入れ替える自分の土地を選んでください（霊地の采配＝全ての自領が対象）"
        : "このターンに通過した自分の土地の駐留式神を、手札の式神と入れ替えます",
      cancelable: true,
  ...
  const res2 = await showDialog({
    ...
    body: `駐留: ${ELEMENTS[cur.element].icon}${esc(cur.name)}（${ELEMENTS[cur.element].name}属性・ST${cur.st}/HP${curHp < cur.hp ? `${curHp}/${cur.hp}` : cur.hp}${elemNote(cur, src)}）<br>` +
      `手札の式神をクリックで交代（召喚コストを支払い、今のカードは手札に戻る。HP全快で駐留）／ 霊力 ${p.magic}G`,
...
async function summonFlow(p, tile) {
  ...
  if (p.isCPU) {
    ...
    const res = await showDialog({
      ...
      body: `式神を召喚して土地を確保できます（霊力 ${p.magic}G）<br>属性が一致すると防衛時にHPボーナス` +
  ...
  const c = CARD_BY_ID[cardId];
  // 霊力の最終チェック（コスト不足での召喚をどの経路からも通さない）
  if (c.cost > p.magic) {
    log(`⚠ 霊力が足りず${c.name}（${c.cost}G）は召喚できない（霊力 ${p.magic}G）`, "warn");
    return false;
  }
  p.magic -= c.cost;
  removeFromHand(p, cardId);
...
// 土地の駐留式神を手札の式神(cardId)と交代する。
// 召喚コストを支払い、元の式神は手札に戻る。新式神はHP全快で駐留。
...
// --- 自分の土地: レベルアップ or 式神交代 or 駐留式神の侵攻（①停止マスの処理）。
//     実行したら true、パスなら false。到達した自領の式神にも命令（交代・侵攻）できる ---
async function ownLandFlow(p, tile) {
  const cost = levelUpCost(tile);
  const cur = CARD_BY_ID[tile.creature.cardId];
  const curHp = tile.creature.hp ?? cur.hp;
  // 到達マスの駐留式神をそのまま隣へ侵攻に出せるか（不動・費用不足・行き先なし・停戦協定中は不可）
  ...
  if (p.isCPU) {
    ...
    const res = await showDialog({
      ...
      body: `駐留: ${ELEMENTS[cur.element].icon}${esc(cur.name)}（${ELEMENTS[cur.element].name}属性・ST${cur.st}/HP${curHp < cur.hp ? `${curHp}/${cur.hp}` : cur.hp}${elemNote(cur, tile)}）／ 霊力 ${p.magic}G<br>` +
        (isFinite(cost) ? `レベルアップ費用 ${cost}G（価値 ${landValue(tile)}G → ${LAND_VALUE[tile.level]}G）<br>` : "この土地は最大レベルです<br>") +
        (canMarchHere ? `🏇 <b>侵攻</b>：駐留式神を隣のマスへ進める（元の土地は空き地に戻る）<br>` : "") +
        (creatures.length ? `または手札の式神をクリックで<b>交代</b>（召喚コストを支払い、今のカードは手札に戻る。HPは全快で駐留）` : ""),
      cards: creatures.map(id => ({ card: CARD_BY_ID[id], disabled: CARD_BY_ID[id].cost > p.magic })),
      peek: true,
      buttons,
    });
  ...
  if (decision.action === "up") {
    ...
    // 到達した自領の式神を隣へ侵攻（②のmarchと同じ処理。元の土地は空き地に戻る）
    ...
    if (!p.isCPU) {
      ...
      if (dst.owner !== null) {
        itemId = await humanPickBattleItem(p, marchCost(cur), `🏇 ${cur.name}に宝具を装備しますか？`);
      }
    }
    if (!dst) return false;
...
async function enemyLandFlow(p, tile) {
  ...
  if (p.isCPU) {
    ...
    const res = await showDialog({
      ...
      body: (protectedTile ? `<b>${truceActive(G) ? "🏳️ 停戦協定により侵略できません" : "🛡️ 結界に守られていて侵略できません"}</b><br>` : "") +
        ...
        (defCard.ab.includes("first") ? `<br><b>⚠ 居合持ち</b>：防衛側が<b>先に</b>攻撃します（HPの低い侵略側は返り討ちに注意）` : "") +
        (defCard.ab.includes("capture") ? `<br><b>⚠ 捕縛持ち</b>：侵略に失敗すると次のターン拘束されます` : "") +
        (defCard.ab.includes("physnull") ? `<br><b>⚠ 物理無効持ち</b>：<b>✨呪力攻撃</b>（呪力攻撃持ち式神／御幣等の装備）以外ではダメージを与えられません` : "") +
        (defCard.ab.includes("physreflect") ? `<br><b>⚠ 物理反射持ち</b>：物理攻撃は<b>そっくり跳ね返されます</b>（✨呪力攻撃なら通る）` : "") +
        (defCard.ab.includes("mimic") ? `<br><b>⚠ 模倣持ち</b>：バトルであなたの式神の<b>基本ST・HP・能力を写し取って</b>戦います（送り込んだ強さがそのまま返ってくる）` : "") +
    ...
    if (res.action === "card") {
      const itemId = await humanPickBattleItem(p, CARD_BY_ID[res.cardId].cost,
        `⚔ ${CARD_BY_ID[res.cardId].name}に宝具を装備しますか？`, false, res.cardId);
  ...
  if (invade) {
    ...
    // 霊力不足などで侵略が成立しなかった → 通行料の支払いへフォールバック
  }
  ...
  await forcePay(G, p, toll, owner, log, landSellChooser(p)); // 払いきれなければ本宮で再起（敗北はしない）
...
// 手札からバトル用宝具を選ぶ（人間用）。ないなら聞かずに null。
// isDefense: 防衛側の応酬か（💨煙玉など防衛側専用宝具は侵略側では選べない・v19）
...
async function humanPickBattleItem(p, committedCost, title, isDefense = false, excludeId = null) {
  ...
  // v25: 二形（hybrid）の式神も「武具として」装備できる＝isEquippable で拾う
  ...
  const res = await showDialog({
    title,
    body: `宝具は使い切りです（残り霊力 ${budget}G）` +
      (items.some(id => CARD_BY_ID[id].asItem) ? "<br>⚔🛡 <b>二形</b>の式神は武具として装備できます（装備すると使い切り）" : "") +
      (isDefense && items.some(id => CARD_BY_ID[id].escape) ? "<br>💨 煙玉＝バトルせず土地を明け渡し、式神は手札へ退避" : ""),
...
async function fightFor(p, tile, attCard, attItem, battleOpts = {}) {
  ...
  if (defender.isCPU) {
    ...
    if (id) {
      defItem = itemFormOf(CARD_BY_ID[id]); // v25: 二形の式神は擬似宝具に変換して装備
  ...
  // 💨煙玉（escape・v19）: 防衛側専用。バトルを行わず土地を明け渡し、式神は手札へ退避する
  ...
  // バトル支援呪術（v20）: 鬨の声/決死の覚悟（攻守どちらでも）・攻城の号令（攻）・護りの風（守）。
  ...
  // バトル演出（結果は先に計算し、カットインで表示だけ流す。実戦は会心あり・土地の援護・百鬼あり）
  ...
  // 吸奪武器（餓鬼の牙＝drainMagic）: バトルで奪った霊力をここで実際に移動する
  // （resolveBattle は状態非破壊のため。相手の所持霊力が上限）
  if (result.attDrain) {
    const amt = Math.min(result.attDrain, defender.magic);
    defender.magic -= amt;
    p.magic += amt;
    if (amt < result.attDrain) log(`（${defender.name}の霊力が足りず、強奪は${amt}Gにとどまった）`, "warn");
    renderPanels(G);
  }
  if (result.defDrain) {
    ...
    p.magic -= amt;
    defender.magic += amt;
    if (amt < result.defDrain) log(`（${p.name}の霊力が足りず、強奪は${amt}Gにとどまった）`, "warn");
    renderPanels(G);
  }
...
async function doInvade(p, tile, cardId, itemId = null) {
  const c = CARD_BY_ID[cardId];
  const attItem = itemId ? itemFormOf(CARD_BY_ID[itemId]) : null; // v25: 二形は擬似宝具に変換
  // 霊力の最終チェック（コスト不足での侵略をどの経路からも通さない）
  const total = c.cost + (attItem ? attItem.cost : 0);
  if (total > p.magic) {
    log(`⚠ 霊力が足りず${c.name}での侵略はできない（必要 ${total}G／霊力 ${p.magic}G）`, "warn");
    return false;
  }
  ...
  if (result.attackerWins) {
    // 倒された防衛式神: 💨煙玉で退避／🔁転生なら手札へ／💨遁走なら空き地へ、通常は捨札へ
    ...
    if (result.defCapture) { // チェインネット等宝具由来の捕縛も含む実効判定（v19）
      p.skipTurn = true;
    ...
    if (toll > 0) {
      ...
      await forcePay(G, p, toll, defender, log, landSellChooser(p)); // 払いきれなければ本宮で再起
      renderAll(G);
    }
...
// tile: バトルが行われた土地（所有者・駐留式神の更新が済んだ後に呼ぶこと）
// defCardId: そのバトルで防衛していた式神のカードid（上書き前に控えておく）
...
// ---------- v25: 💨遁走（escaper）＝防衛に敗れても消滅せず、空いている霊地へ逃げ延びる ----------
...
function tryEscapeToEmptyLand(owner, cardId) {
  ...
  if (empties.length === 0) {
    log(`💨 ${card.name}は逃げ延びようとしたが、空いている霊地が無かった…`, "warn");
    return false;
  }
  ...
  SFX.summon();
  log(`💨 ${card.name}の遁走！ 敗れても消えず、${tileName(dst)}へ逃げ延びて${owner.name}の霊地にした`, "battle", { toast: true });
  return true;
}
...
// ---------- 式神侵攻（march） ----------
// 停止マスのアクションを放棄し、このターン通過した自軍式神を隣のマスへ進める。
// 空き地なら無血占領、敵地ならバトル（負ければ式神消滅）。どちらも元の土地は空き地に戻る。
...
async function humanMarchFlow(p) {
  ...
  if (sources.length === 1) {
    ...
    src = await humanPickTileOnMap(sources, {
      title: "🏇 式神侵攻 — 出撃元を選択",
      body: "このターンに通過した自分の式神を、隣のマスへ進められます（行軍費を支払う。元の土地は空き地に戻る）",
      cancelable: true,
  ...
  let itemId = null;
  if (dst.owner !== null) {
    itemId = await humanPickBattleItem(p, marchCost(card), `🏇 ${card.name}に宝具を装備しますか？`);
  }
...
async function doMarch(p, src, dst, itemId = null) {
  ...
  // 霊力の最終チェック（行軍費＋宝具費が払えなければ侵攻不可）
  const item = itemId ? CARD_BY_ID[itemId] : null; // コスト判定はカード本体のコストでよい（二形も同額）
  if (cost + (item ? item.cost : 0) > p.magic) {
    log(`⚠ 霊力が足りず${card.name}は侵攻できない（必要 ${cost + (item ? item.cost : 0)}G／霊力 ${p.magic}G）`, "warn");
    return false;
  }
  p.magic -= cost;
  SFX.dice();
  ...
  if (dst.owner === null) {
    ...
    // 🐏破城（siegebreak・v25）: ぶつかる前に城壁を砕く＝相手の霊地レベルを1下げてからバトルに入る。
    ...
    // march は盤上の式神の出撃＝🌱成長段階・無力化・出撃元（百鬼の自己除外）をバトルへ引き継ぐ（v19/v20）
    ...
    if (result.attackerWins) {
      ...
      // 引き分け・両者生存: 侵攻側は傷を負って元の霊地へ撤退。霊地の変動なし
      src.creature.hp = woundedHp(result.attHp, result.attExtra, maxHpOf(src.creature));
...
// ---------- 盤面のマス情報（クリックで確認） ----------
// 盤面のマスをクリックすると、そのマスの情報——特に霊地に駐留する敵式神の
...
function showTileInfo(tile) {
  ...
  if (tile.type === "LAND") {
    const ownerTxt = tile.owner === null ? "空き地"
      : `<b style="color:${PLAYER_COLORS[tile.owner]}">${P_ICONS[tile.owner]} ${esc(G.players[tile.owner].name)}の霊地</b>`;
    ...
    if (ov) parts.push(ov.kind === "sanctuary"
      ? `🛡️ 結界に守られている（侵略・侵攻・敵呪術の対象にならない）`
      : `🕸️ 罠が仕掛けられている（術者以外が通過・停止すると足止め）`);
  } else {
    const descs = {
      CASTLE: "🏯 本宮 — 鳥居を規定数そろえて通過・停止すると周回ボーナス（霊力＋全回復）。ぴったり停止で霊地の采配。目標資産で帰還すれば勝利！",
      GATE:   `⛩️ 鳥居 — くぐると+${RULES.gateBonus}G。規定数そろえて本宮へ戻ると周回ボーナス`,
      CARD:   "🎴 札マス — 止まるとカードを1枚引く",
      MAGIC:  `💎 霊力マス — 止まると+${RULES.magicTileG}G`,
      WARP:   "🌀 神隠しマス — 止まると対のマスへ移動する",
      MAGMA:  `🌋 火口マス — 止まると霊力を失う（-${RULES.magmaLoss}G）`,
      BOOST:  "💨 疾風マス — 止まるとさらに2マス進む",
      FORTUNE: "🎋 おみくじマス — 止まるとおみくじ！ 大吉+300G／吉+150G／2枚ドロー／次の賽2倍／🌀時空の渦（進行方向が反転）／凶-100G のどれかが起きる",
      SPRING: "♨️ 霊泉マス — 止まると自軍式神のHPが全回復＋60G",
    };
...
async function startSealed(stageIdx) {
  ...
  const ok = await showDialog({
    ...
    buttons: [
      { label: "🎁 文箱を開封する", value: "open", primary: true },
...
async function titleScreen() {
  ...
  while (true) {
    ...
    if (res === "sealed") {
      // 🎁 封符戦: 全ステージから盤面を選ぶ → その場で文箱開封 → プールから構築 → 1戦
      while (true) {
...
// ---------- 遊び方ヘルプ ----------
// passive:true（v23）: 三つ巴では閲覧中もCPUの手番が進み、防衛宝具選択などの進行ダイアログが
...
function showHelp() {
  return showDialog({
    ...
    title: "❓ 遊び方",
    body: `
      <b>勝利条件</b>: 総資産（霊力＋土地価値）がステージの目標に達した状態で🏯本宮に到達（凱旋）する。
      ...
      敵の土地は<b>通行料の支払い or 式神で侵略（バトル）</b>のどちらか。
      <b>侵略に勝てば土地を奪取し通行料は不要。ただし侵略に敗れると、その土地に留まった扱いで通行料も徴収される</b>
      （足止めの罠で止められて侵略に敗れた場合も同じ）。
      鳥居・霊力・本宮・カードなどのマスは受け身のイベントだけが起きる。<br>
      <b>② 通過アクション（①で能動行動しなかったターンだけ・1つ）</b>: <b>①で能動的な行動（召喚・レベルアップ・交代・侵攻・侵略）をしなかった</b>ターンに限り、
      このターンに<b>通過した</b>自分の霊地について次のどれか1つを行える。<br>
      <b>＝そのターンの能動行動は1回まで。①で行動すれば②は無し。①が受け身マス／通行料のみ／パス／召喚できる手札が無い場合に、権利が②へ回る</b>（実際に選べる項目だけボタンが出る）。<br>
      ・<b>式神侵攻</b> … 通過した自分の式神を隣のマスへ進める。
        <b>行軍費は式神のコスト×25%（最低${MARCH_COST_MIN}G）と安く、気軽に仕掛けられる</b>。
        空き地なら無血占領、敵地ならバトル（勝てば制圧／引き分けなら元の土地へ撤退／負ければ消滅し元の土地も失う）<br>
      ・<b>式神交代</b> … 通過した自分の土地の駐留式神を、手札の式神と入れ替える（召喚コストを支払う）<br>
      ・<b>通過地レベルアップ</b> … 通過した自分の土地を1つレベルアップする<br>
      ※<b>自分の土地に到達</b>したときは、①でレベルアップ・交代に加えて<b>駐留式神の侵攻</b>も選べる。<br>
      ※<b>出発マス（ターン開始時にいたマス）は②の対象外</b>（前のターンの到達アクションで命令できたため）。<br>
      ※霊地・式神の選択では、盤面の各マスに振られた<b>#番号</b>で選択肢とマスが対応します。<b>「👁 盤面から選ぶ」</b>で盤面を表示し、<b>光っているマスを直接クリック</b>しても選べます。<br>
      ※選択中でないときに<b>盤面のマスをクリック</b>すると、そのマスの情報——<b>駐留式神の能力・現在HP・実効防衛値（援護/土地の加護/守護）・通行料</b>——を確認できます（敵の霊地もOK）。<br>
      ...
      ・自分の土地 … 土地レベルアップ、または式神を手札と<b>交代</b>できる<br>
      ・敵の土地 … 通行料を支払う or 式神で侵略バトル<br>
      ・🎴札＝1枚ドロー ／ 💎霊力＝霊力を獲得 ／ ⛩️鳥居＝くぐるとボーナス<br>
      ・🌀神隠し＝対のマスへ移動 ／ 🌋火口＝霊力を失う ／ 💨疾風＝さらに2マス進む<br>
      ・🎋<b>おみくじ</b>＝運試し（大吉+300G／吉+150G／2枚ドロー／次の賽2倍／🌀時空の渦＝進行方向が反転／凶-100G） ／
      ♨️<b>霊泉</b>＝自軍式神全回復＋60G<br><br>
      ...
      <b>レア度</b>: カードには★（並）〜★★★★（極）のレア度があり、文箱では低いレア度ほど出やすい。<br>
      ...
      <b>🎲 賽</b>は画面右下の丸いボタン（＝直前の出目もここに出る）。PCでは<b>Space / Enterキー</b>でも振れる。
      情報窓・手札・賽はすべて盤面の外に置いてあるので、<b>盤面が隠れることはない</b>。<br>
      <b>🔎 盤面の拡大</b>: ヘッダーの<b>－／＋</b>で拡大縮小、<b>⛶ 全体</b>で全体が収まる倍率へ、<b>100%</b>をクリックで等倍（＝盤面エリアぴったり）。
      Ctrl+マウスホイール、スマホは2本指ピンチでも調整できる。<br>
      <b>🔔 ポップアップ通知</b>: <b>呪術の効果・特性の発動・機能停止（呪術封じ・足止め・無力化など）・
      通行料・霊力不足</b>など「影響のあった出来事」は、📜ログと同じ文言を画面上部に短く表示してすぐ消える。
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  // 盤面のマスをクリックして情報を確認（駐留式神の能力・実効防衛値など）。
  // 決定待ちのダイアログ中・霊地選択中（光っているマスを選ぶモード）は開かない
  ...
  // キーボード操作（PC）: Space / Enter＝メインの操作（賽を振る）、M＝マップ確認、L＝ログ
